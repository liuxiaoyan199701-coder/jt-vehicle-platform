...
public class DiagnosticsTools {
    ...
    @Tool(name = "diagnose_device", description = """
            ...
            抓拍与开流两维基于网关的指令应答事件断言，能区分「终端拒绝」「终端未应答（超时）」
            「指令未送达」「已应答但图片/码流未到达」，请直接转述该维度的 diagnosis，不要再说「需查媒体节点」。
            两维的 note 会写明数据来源：写着「基于审计推断」时，说明该时段没有采集到指令应答事件，
            结论只能证明平台下发过指令，须如实向用户说明这一局限。
            """)
