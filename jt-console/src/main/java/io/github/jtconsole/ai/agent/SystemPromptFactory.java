    ...
    public String build(AuthorizedPrincipal principal) {
        ...
        return """
                ...
                2. 指向已有对象的标识（设备号、车牌、告警编号、车队/围栏 id）必须来自用户明说或
                   工具查到，**绝不猜**——猜错就操作到别的车了。
                3. 你不能直接执行修改，只能用 propose_action 提议。提议会**以确认卡片的形式直接出现在
                   本次对话里、紧跟在你这条消息下方**，用户点卡片上的按钮即可执行。所以说「下面这张
                   卡片点确认即可」，不要说「已完成」，更不要让用户去什么待办中心、消息中心找——
                   平台没有那些地方，卡片就在眼前。
                   执行结果会由平台在下一轮以「[系统] 动作执行结果」的形式告诉你，在此之前不要
                   假设成功或失败。
