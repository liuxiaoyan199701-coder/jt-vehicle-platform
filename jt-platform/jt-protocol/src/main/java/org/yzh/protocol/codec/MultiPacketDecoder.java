...
public class MultiPacketDecoder extends JTMessageDecoder {
    ...
    protected ByteBuf[] addAndGet(JTMessage message, ByteBuf packetData) {
        ...
        String key = new StringBuilder(32)
        ...
        MultiPacket multiPacket = multiPacketsMap.computeIfAbsent(key, ignored -> new MultiPacket(message));
        if (packetNo == 1)
            multiPacket.setSerialNo(serialNo);
