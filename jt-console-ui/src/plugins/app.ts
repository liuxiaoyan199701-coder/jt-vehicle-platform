...
export function setupAppVersionNotification() {
  ...
  const checkForUpdates = async () => {
    ...
    // The notification API is registered by AppProvider after mount. If a check lands before
    // that, `window.$notification` is still undefined; setting `isShow` first would leave it
    // stuck at true forever and silently disable every later check. Bail out instead and retry
