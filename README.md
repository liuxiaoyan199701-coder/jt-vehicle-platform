...
| 设备日志 | `GET /api/device-logs` | 按 `deviceId`（必填）与 `start`、`end`、`direction`、`msgId`、`keyword`、`page`、`pageSize` 检索报文日志 |
| 日志详情 | `GET /api/device-logs/{id}` | 单条记录的原始帧 hex 与解析 JSON |
| 终端清单 | `GET /api/terminals` | 连接过网关的终端，支持 `keyword`、`archived`、`online`、`start`、`end`、`page`、`pageSize` |
| 终端建档 | `POST /api/terminals/{deviceId}/archive` | 把未建档终端转为车辆档案，与车辆档案页新建走同一套校验 |
...
生产环境的 `root: INFO` 下，网关那条 DEBUG 级的 hex 打印根本不落盘——也就是说
**原始帧此前没有任何地方留存**。排查设备问题只能 ssh 上服务器 grep 文件日志，
AI 助手做设备体检时也拿不到报文级证据。设备日志中心把这段补上。

三类记录进同一张表，一次查询就是一台设备的完整时间线：

| 方向 | 内容 |
|---|---|
| `UP` | 设备上行报文，含原始帧 hex 与解析后的报文正文 |
| `DOWN` | 平台下行指令，记录完整成帧字节（分包合并为一条） |
| `CONNECTION` | 上下线、注册与鉴权结局，由连接事件镜像而来 |

几条值得知道的设计取舍：

- **解码失败的帧照样入库**。那正是最需要原始字节的一类——只有那串 hex 能说明终端到底发了什么。
  这类记录带 `decodeError` 标记，解析正文为空。
- **0x9206 的一次性 FTP 口令不落盘**。原始帧里的明文没法只抹掉一段，整条 hex 都不留，
  解析结果里口令字段替换为 `***`。
- **日志库是独立的 SQLite 文件**（默认 `data/jt-console-logs.db`），与业务库物理隔离。
  报文是持续高频写，与业务写抢同一把写锁会直接体现为接口变慢。
- **投递是 BEST_EFFORT**。日志与位置报文同量级，走可靠投递会在控制台停机时打爆落盘 spool；
  日志丢几条可容忍，主链路与磁盘不可。
- 单条 hex 与解析 JSON 在**网关侧**就截断（各默认 8192 字符），超长内容进不了投递队列。
- 默认保留 14 天，由定时任务分批清理。

AI 助手同时获得两个只读工具：`query_device_logs` 取摘要行，`get_device_log_detail`
按 id 取单条的原始 hex 与解析正文，与页面共用同一查询服务和同一租户隔离。

```yaml
jt:
  signal:
    message-log:            # 网关侧采集，可整体关闭
      enabled: true
      max-hex-chars: 8192
      max-json-chars: 8192
  console:
    device-log:
      db: data/jt-console-logs.db
      retention: 14d
```

### 终端管理

平台原本只有人工维护的车辆档案，而档案是「人先建、设备后连」的顺序。生产用的
`allow-all` 鉴权模式下，终端自报一个终端 ID 就能注册上线——于是存在一类平台完全
看不见的设备：**连上来了、在发报文，但没人给它建过档**。

终端管理消费注册与鉴权消息，把每台连接过网关的终端自动入册，展示它自报的终端 ID、
制造商、型号、车牌与协议版本，并标注是否已建档。未建档终端可一键转成车辆档案，
用自报车牌预填表单。AI 侧对应只读工具 `query_terminals`。

两点容易误读的地方，页面上也照此措辞：

- **自报车牌与档案车牌分两列显示**，不合并。自报值是终端自己填的，可以是错的；
  档案值是运维确认过的。合并之后就无从判断眼前这个车牌可不可信，而这恰恰是建档时最需要的。
- **「最近注册/鉴权」不等于「最近在线」**。台账只在注册或鉴权时更新，长连不断的终端
  不会刷新它；当前是否在线由单独一列给出。

未建档终端没有租户归属，因此只对平台管理员可见；租户账号看到的集合与其在车辆档案里
能看到的一致。
...
DeepSeek 没有视觉能力。要让助手能看懂**抓拍照片**和**用户在对话里发的图**，另配一个视觉模型——
它只负责「看图出文字」，描述再作为普通文本进入主模型的上下文，主模型全程不接触图像：

```bash
export JT_AI_VISION_BASE_URL=<OpenAI 兼容的服务地址>   # 只需要 /v1/chat/completions
export JT_AI_VISION_API_KEY=<该服务的密钥>
export JT_AI_VISION_MODEL=<具备视觉能力的模型名>
```

三项缺任意一项即整条视觉链路关闭：抓拍查询退化成只有元数据，对话里的图片上传接口明确拒绝——
少一个功能，而不是报错。

界面上不体现两个模型的存在：抓拍的画面描述折在抓拍查询结果里，用户发的图在对话开始前就已识别完，
不会多出「正在识别图片」这类中间状态。

> 视觉模型按图像面积计费。平台默认单次最多送 4 张、长边压到 1280 像素，
> 可用 `JT_AI_VISION_MAX_IMAGES` 与 `JT_AI_VISION_MAX_EDGE` 调整。
...
- [808 的手机号涨到 20 位，而 1078 还是 12 位](docs/articles/04-jt1078-sim-width.md)
  —— 标准留下的缺口、野生设备的三种做法，以及「流连上了但画面是花的」该怎么查
